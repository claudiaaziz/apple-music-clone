	...
	const handlePressAction = (id: string) => {
		match(id)
			.with('add-to-favorites', async () => {
				...
				// if favorites is currently in queue add new fav track to queue
			...
			.with('remove-from-favorites', async () => {
				...
				// if favorites is currently in queue remove track from queue
				if (activeQueueId?.startsWith('favorites')) {
			...
			.otherwise(() => console.warn(`Unkown menu action: ${id}`))
	}
